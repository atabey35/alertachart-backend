...
router.delete('/custom', optionalAuth, async (req, res) => {
  try {
    ...
    // 🔥 CRITICAL: For guest users, if no userId from cookie/token, try to find user by device_id and userEmail
    if (!userId && deviceId) {
      console.log('[Alerts DELETE Custom] 🔓 Guest user detected, trying to find user:', {
        deviceId: deviceId.substring(0, 20) + '...',
        userEmail: userEmail || 'not provided',
      });
      ...
      try {
        ...
        // Fallback: email if provided
        if (guestUsers.length === 0 && userEmail) {
          console.log('[Alerts DELETE Custom] ⚠️ Guest user not found by device_id, trying email...');
          guestUsers = await sql`
            SELECT id, email, plan, expiry_date, trial_started_at, trial_ended_at, device_id
            FROM users 
            WHERE email = ${userEmail}
        ...
        if (guestUsers.length > 0) {
          userId = guestUsers[0].id;
          console.log('[Alerts DELETE Custom] ✅ Guest user found:', {
            userId,
            email: guestUsers[0].email,
            plan: guestUsers[0].plan,
          });
        } else {
          console.log('[Alerts DELETE Custom] ⚠️ Guest user not found');
        }
      } catch (guestError) {
        console.error('[Alerts DELETE Custom] ❌ Error finding guest user:', guestError);
      }
    }
